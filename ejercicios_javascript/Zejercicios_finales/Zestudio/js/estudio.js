...
const frase = window.prompt('Escribe frase y veremos si es un palíndromo, schaaval!');//Pido una frase

function IsItPalindrome(texto) { //Esta función confirmará si es o no un palíndromo

    if (!texto) {
        return false;
    }

    let cadena = texto.replace(/ /g, ''); //Meto en una variable la frase sin espacios en blanco, usando replace() y la RegEx / /g
    cadena = cadena.toLowerCase();  //Elimino mayúsculas usando string.toLowerCase()
    
    for (let i = 0; i < cadena.length / 2; i++) { //Bucle FOR de la mitad de la length ya que comparas los índices extremos hasta llegar al centro(la mitad)
        
        if (cadena[i] !== cadena[cadena.length - (i + 1)]) { //A la length se le resta i+1 para que coincida con el índice buscado.
            return false; //uso !== (distinto de) para que salte si hay una sóla dif, si no la hay es palín fijo.
        }   //Pero si usase === (igual que) fallaría ya que cuando hubiese una sóla coincidencia el return se activaría (SinliteraS)
    }
    return true;
}

if (!frase) {
    console.log('Escribe un texto majadero!');
}



if (IsItPalindrome(frase)) {
    console.log(`Bravo! El texto: ${frase} SÍ es palíndromo`);
} else {
    console.log(`El texto: ${frase} NO es palíndromo`);
} 
//---------------------------FIN EJERCICIO UNO---------------------------------------
*/
...
function pedirConcepto() {
let concepto;
    while (!concepto) {
       concepto = window.prompt('Introduce el concepto.');
    }
    return concepto;
}
...
while (!base) {
    ...
    base = window.prompt('Introduce la base imponible del concepto');
...
function masConceptos() {
    return window.confirm('¿Desea añadir otro concepto?');    
}
...
let mas = true;
    ...
    while (mas) {
        const aniadir = {};
        aniadir.descripcion = pedirConcepto();
        aniadir.base = pedirBase();
        concepBase.push(aniadir);
        mas = masConceptos();
    }
let totalBases = 0;

for (let i = 0; i < concepBase.length; i++) {
    console.log(`Concepto: ${concepBase[i].descripcion} -- ${concepBase[i].base} euros`);
    totalBases += concepBase[i].base;
}
console.log(`Total bases imponibles: ${totalBases} euros`);
console.log(`21% iva: ${totalBases * 0.21} euros`);
console.log(`Total factura: ${totalBases * 1.21} euros`);
...
/*XXXXXXX   
    ...
    function eliminarTarea() { //borra elementos del ARRAY listaTareas por medio del método SPLICE
        ...
        listaTareas.splice(numTarea2 - 1, 1);          //Al restarle 1 al valor de TASK obtenemos el índice del ARRAY (ya que estos van del 0 al n)
